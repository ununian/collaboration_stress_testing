...
pub async fn create_peer(info: DocInfo, target_count: usize, id: String, thread_id: usize) -> () {
    let id = Arc::new(id);
    println!("id: {}", id.clone());
...

    {
        ...
        let handle2 =
            tokio::spawn(create_yjs_connection(
                Arc::clone(&doc),
                info.clone(),
                            ...
                            if let Some(success) = rx.recv().await {
                                ...
                                if let Some(text_ref) = text_ref {
                                    ...
                                    let sub = text_ref.observe(move |txn, event| {
                                        ...
                                        if text.ends_with(
                                            format!("ping_{}_{} ", thread_id, *count_clone)
                                                .as_str(),
                                        ) {
                                            if let Err(e) = tx_clone.lock().unwrap().try_send(
                                                format!("pong_{}_{} ", thread_id, *count_clone),
                                            ) {
                                                println!("添加文字发送消息失败: {:?}", e);
                                            }
                                            *count_clone += 1;
...

                                    {
                                        ...
                                        {
                                            let mut count_clone = count_clone.lock().unwrap();
                                            if let Err(e) = tx.lock().unwrap().try_send(
                                                format!("pong_{}_0 ", thread_id).to_string(),
                                            ) {
                                                println!("添加文字发送消息失败: {:?}", e);
                                            }
                                            *count_clone += 1;
